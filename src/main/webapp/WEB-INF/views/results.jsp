...
<body>
	<div
		style="text-align: center; background-image: url('https://e0.pxfuel.com/wallpapers/917/663/desktop-wallpaper-displaying-18-for-movie-theater-background-for-your-mobile-tablet-explore-theater-background-home-theater-home-theater-for-movie-theater.jpg'); background-repeat: no-repeat; background-size: cover;">
		<br /> <br /> <br /> <br />

		<c:choose>
				<c:when
					test="${not empty filmList && success == true && multiple == true}">
		<h1 style="color:cornsilk">Matches for the keyword query: ${identifier} </h1>
		<c:forEach var="films" items="${filmList}">
		<p style="color:cornsilk">${films.filmId} | ${films.title } </p><br />
					</c:forEach>
				</c:when>
		...
		Failed to insert Film into Database
					...
					<c:if test="${film.actors != null }">
						...
						<ul>
							<c:forEach var="actor" items="${film.actors }">
								<li style="color: cadetblue">${actor.firstName }
									${actor.lastName }</li>
							</c:forEach>
						</ul>
					</c:if>
			...
			<c:when
				test="${film == null && multiple == true && success == false}">
				<h1>Your query found multiple matches. Please narrow the search
					to one Film.</h1>

			</c:when>

			<c:when
				test="${film == null && multiple == false && success == false}">
				<h1>Your query did not find any matches</h1>
			</c:when>
		</c:choose>
		<br>

		<form action="delete.do" method="POST" style="color: gold">
			Delete Film: <input type="text" name="filmId" value="${film.filmId}">
			<input type="submit" value="DELETE FROM DATABASE">
		</form>
		<form action="update.do" method="POST" style="color: gold">
